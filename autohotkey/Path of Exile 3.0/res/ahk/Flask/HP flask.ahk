; ==============================================
; Надо разобраться с:
; 1: Скрипт автопражатия скилов, вообще не вникал, искать под Sleepfunction в начале скрипта
...
; ВАЖНО: При изменении или добавлении биндов надо проверять все, т.к. есть строчки типа if (Vkeys < 1 and Vkeys > 10) и при добавлении новых вариантов переменных надо их подправлять
;============================================================================================
; Переменные
; =========================
; Основные переменные:
...
; В списке "Надо разобраться с:" пункт 1, нихуя не помню как оно работает.
autocast() {
	If Vkeys > 0
	{
		if Var_auto_skils = 0
...
sc29::
/* 	В списке "Надо разобраться с:" пункт 5.
if Vkeys != 6
{
	SendInput, 5
}
...
if (Vkeys < 1 and Vkeys > 10)			; дефолт (~)
{
	SendInput, {sc29}
}
...
/* 
; =================================================================================
; В списке "Надо разобраться с:" пункт 6, там описано что это за хрень.
...
else if (Vkeys < 1 and Vkeys > 10)		; W (дефолт)
{
	SendInput, {sc11}
}
return
...
; ~+scA::
; =====
; Закомментировать все в этом блоке для доступности всех биндов. Если какое-то значение Vkeys задано (не закомментированная строка), то при нажатии на F8 значение Vkeys станет на 1 больше. К примеру если не закомментирована строка "Vkeys := 0 ; 1&2 = 1/2, 3&d = 3+4+5, ~ = 5" то Vkeys станет 1. В строке указано какие бинды выставятся.
...
Vkeys := 0 ; Vkeys станет 2, бинды: 1&2 = 1/2, 3&d = 3+4+5, ~ = 5
; Vkeys := 1 ; для тестов
; =====
