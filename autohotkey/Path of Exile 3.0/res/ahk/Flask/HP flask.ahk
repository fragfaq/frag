...
; --------------------------------------------------------------------------------
; Воспроизведение звука при акивации скрипта, если хоткей нажат случайно
...
return
;==============================================
; Сброс биндов на дефолтные и отключение автокаста при нажатии F7, Enter, Ctrl+Enter, и комбинаций Ctrl+Shift+кнопка мыши (как правая так и левая)
; Бинд разделен на две части, что б звук воспроизводился только при активном окне PoE и только на два бинды Ctrl+Shift+кнопка мыши
; Это сделано потому, что на другие хоткеи сложно нажать случайно, но сами они нажимаются часто по прямому назначению и постоянный звук бы раздражжал.
; Если хоткей был нажат вне активного окна игры, то он все равно прозвучит при активации. Возможно это тоже можно пофиксить, но мне лень =)
~+RButton::
~+LButton::
IfWinNotActive, ("ahk_class POEWindowClass")
		{
